...
+ (instancetype)voiceBox {
    return [[self alloc] initWithFrame:CGRectMake(0, 70, DDYSCREENW, FFChatBoxFunctionViewH)];
}
...
- (instancetype)initWithFrame:(CGRect)frame {
    if (self = [super initWithFrame:frame]) {
        ...
        // 对讲视图
        _voiceTalkView = [DDYVoiceTalkView viewWithFrame:DDYRect(self.ddy_w, 0, self.ddy_w, self.ddy_h)];
        _voiceTalkView.playBlock = ^() { [weakSelf setupPlayView]; };
...
- (void)setupPlayView {
    ...
    DDYInfoLog(@"111111:%ld",[DDYRecordModel shareInstance].duration);
}
